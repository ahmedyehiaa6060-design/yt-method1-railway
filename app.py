...
def cut_video(req: CutRequest):
    ...
    # لا نقوم بإجبار صيغة mp4 هنا في الاسم، بل نترك yt-dlp يحدد الصيغة الأصلية %(ext)s
    outtmpl_path = os.path.join(TEMP_DIR, f"{file_id}.%(ext)s")

    # إعداد ديناميكي للصيغ ودقة القص بناء على الجودة المطلوبة لتفادي الانهيار OOM وتلف الفيديو
    if req.quality <= 1080:
        ...
        # للجودات العالية جداً (2K/4K): نطلب أفضل الجودات المتاحة (غالباً WebM) ونسخ البث مباشرة
        format_str = f'bestvideo[height<={req.quality}]+bestaudio/best[height<={req.quality}]'
        merge_fmt = 'webm' # نخزنها كـ webm لتطابق ترميز VP9/AV1 الأصلي
        force_keyframes = False
        print(f"⚠️ High quality ({req.quality}p) requested. Using native formats and disabling force_keyframes to prevent container mismatch.")
    ...
    # تحديد نوع الميديا ديناميكياً لإرجاعها بشكل صحيح للمستخدم
